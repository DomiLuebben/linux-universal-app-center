...
LUT_CONTAINER="$(docker create --network=host -e LUT_INTEGRATION_ALLOW_MUTATIONS=1 -e QT_QPA_PLATFORM=offscreen -e LANG=C.UTF-8 "$LUT_IMAGE" sh -c '
    apt-get update &&
    cmake -S /app -B /app/build && cmake --build /app/build -j$(nproc) &&
    ctest --test-dir /app/build --output-on-failure &&
    QT_QPA_PLATFORM=offscreen /app/build/linux-update-tool/linux-update-tool --check-qml --replay /app/tests/fixtures/small-update.jsonl &&
