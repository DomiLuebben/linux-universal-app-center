...
echo "=== [1/5] Building linux-universal-app-center ==="
...
QT_QPA_PLATFORM=offscreen "${ROOT_DIR}/build/linux-app-store/linux-universal-app-center" --check-qml --replay "${ROOT_DIR}/tests/fixtures/small-update.jsonl"

echo "=== [4/5] Validating desktop entry and AppStream metadata ==="
desktop-file-validate "${ROOT_DIR}/data/org.linuxuniversalappcenter.desktop"
appstreamcli validate --no-net "${ROOT_DIR}/data/org.linuxuniversalappcenter.metainfo.xml"
...
HARDCODED_COLORS=$(grep -rnE '#[0-9a-fA-F]{3,8}' "${ROOT_DIR}/linux-app-store/qml/" 2>/dev/null | grep -v "Vorläufig für M0" || true)
