...
void StoreModel::onAppStateChanged(const QString &appKey)
{
    ...
    for (int i = 0; i < m_items.size(); ++i) {
        if (m_items[i].appKey == appKey) {
            ...
            emit dataChanged(index(i), index(i), { ActionStateRole, IsInstalledRole });
            break;
        }
    }
}
...
void StoreModel::rebuildItems()
{
    ...
    if (m_packagesOnly) {
        ...
        for (const auto &offer : offers) {
            ...
            InstalledState inst = m_appStore->installedState(pkg.name);
            item.isInstalled = inst.isFullyInstalled;
            item.actionState = item.isInstalled ? QStringLiteral("Installed") : QStringLiteral("Available");
    ...
    // Filter anwenden: Kategorie & Installiert
